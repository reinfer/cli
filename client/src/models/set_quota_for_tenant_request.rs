...
pub struct SetQuotaForTenantRequest {
    /// Limit to which to set the quota. Requests to create a new resource will fail if that quota is exceeded.
    #[serde(rename = "hard_limit")]
    pub hard_limit: i32,
    /// Limit to which a user is able to automatically increase the quota. Specify None for the default limit.
    #[serde(rename = "auto_increase_up_to", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub auto_increase_up_to: Option<Option<i32>>,
}

impl SetQuotaForTenantRequest {
    pub fn new(hard_limit: i32) -> SetQuotaForTenantRequest {
        SetQuotaForTenantRequest {
            hard_limit,
            auto_increase_up_to: None,
        }
    }
}
