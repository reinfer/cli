...
pub struct Theme {
    #[serde(rename = "theme_comments")]
    pub theme_comments: Vec<models::ThemeComment>,
    /// Populated if LLM features are enabled. These label names suggested by the LLM for the theme.
    #[serde(rename = "suggested_labels")]
    pub suggested_labels: Vec<models::SuggestedLabel>,
    /// Populated if LLM features are enabled. The reasoning behind the suggested labels.
    #[serde(rename = "suggested_labels_reasoning", skip_serializing_if = "Option::is_none")]
    pub suggested_labels_reasoning: Option<Box<models::LabelSuggestionReasoning>>,
}

impl Theme {
    pub fn new(theme_comments: Vec<models::ThemeComment>, suggested_labels: Vec<models::SuggestedLabel>) -> Theme {
        Theme {
            theme_comments,
            suggested_labels,
            suggested_labels_reasoning: None,
        }
    }
}
