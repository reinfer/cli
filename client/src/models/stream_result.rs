...
pub struct StreamResult {
    /// The comment itself---the data WILL reflect changes that have occurred since the data was first uploaded and the time this call was made.
    #[serde(rename = "comment")]
    pub comment: Box<models::Comment>,
    #[serde(rename = "prediction", deserialize_with = "Option::deserialize")]
    pub prediction: Option<Box<models::StreamCommentPrediction>>,
    /// An opaque token that can be used to advance the trigger past this result. If used, the next results to be returned will exclude all results that precede this one in this list.
    #[serde(rename = "continuation")]
    pub continuation: String,
}

impl StreamResult {
    pub fn new(comment: models::Comment, prediction: Option<models::StreamCommentPrediction>, continuation: String) -> StreamResult {
        StreamResult {
            comment: Box::new(comment),
            prediction: if let Some(x) = prediction {Some(Box::new(x))} else {None},
            continuation,
        }
    }
}
