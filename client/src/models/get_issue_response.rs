...
pub struct GetIssueResponse {
    #[serde(rename = "status")]
    pub status: Status,
    /// Pydantic model with sane defaults and better Optional handling.
    #[serde(rename = "issue")]
    pub issue: Box<models::Issue>,
    #[serde(rename = "alerts")]
    pub alerts: Vec<models::Alert>,
}

impl GetIssueResponse {
    pub fn new(status: Status, issue: models::Issue, alerts: Vec<models::Alert>) -> GetIssueResponse {
        GetIssueResponse {
            status,
            issue: Box::new(issue),
            alerts,
