...
pub struct IxpPredictExtractionsResponse {
    #[serde(rename = "status")]
    pub status: Status,
    #[serde(rename = "extraction_status")]
    pub extraction_status: models::IxpExtractionStatus,
    #[serde(rename = "extraction", deserialize_with = "Option::deserialize")]
    pub extraction: Option<Box<models::IxpTaxonomyExtraction>>,
}

impl IxpPredictExtractionsResponse {
    pub fn new(status: Status, extraction_status: models::IxpExtractionStatus, extraction: Option<models::IxpTaxonomyExtraction>) -> IxpPredictExtractionsResponse {
        IxpPredictExtractionsResponse {
            status,
            extraction_status,
            extraction: if let Some(x) = extraction {Some(Box::new(x))} else {None},
        }
    }
}
/// 
