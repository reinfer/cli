...
pub struct ExtractionPrediction {
    ...
    pub label: Option<Box<models::LabelNameOnlyResponse>>,
    #[serde(rename = "captures")]
    pub captures: Vec<models::MoonFormCaptureFieldsPrediction>,
}

impl ExtractionPrediction {
    pub fn new(label: Option<models::LabelNameOnlyResponse>, captures: Vec<models::MoonFormCaptureFieldsPrediction>) -> ExtractionPrediction {
        ExtractionPrediction {
            label: if let Some(x) = label {Some(Box::new(x))} else {None},
            captures,
        }
    }
}
