...
fn test_create_with_empty_org_fails() {
    ...
    assert!(
        output.contains("405 Method Not Allowed") || output.contains("expected owner/name, got '/bucket-name-with-empty-org'"),
        "{}",
        output
    );
}
...
fn test_create_with_empty_bucket_name_fails() {
    ...
    // Updated to match actual error format - API validation changed from client-side to server-side
    assert!(
        output.contains("404 Not Found") || output.contains("expected owner/name, got 'org-without-bucket-name/'"),
        "{}",
        output
    );
}
