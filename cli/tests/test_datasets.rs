...
use openapi::models::{
    BuiltinLabelDef, BuiltinLabelDefName, Dataset, DatasetFlag, EntityDef, LabelDef, LabelGroup,
    ModelFamily, MoonFormFieldDef, Source,
};
        ...
        fn from(dataset: Dataset) -> DatasetInfo {
            DatasetInfo {
                ...
                label_defs: dataset
                    .label_defs
                    .into_iter()
                    .map(normalize_moon_form_ids)
                    .collect(),
                label_groups: dataset
                    .label_groups
                    .into_iter()
                    .map(|mut group| {
                        group.label_defs = group
                            .label_defs
                            .into_iter()
                            .map(normalize_moon_form_ids)
                            .collect();
...
fn test_create_dataset_with_gen_ai() {
    ...
    assert_eq!(&dataset_gen_ai_false_info.name, dataset_gen_ai_false.name());
