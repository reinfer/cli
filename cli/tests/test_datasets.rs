...
use openapi::models::{Dataset, DatasetFlag, Source, ModelFamily, EntityDef,
                        LabelDef, LabelGroup, MoonFormFieldDef, BuiltinLabelDef, BuiltinLabelDefName};
use serde_json::json;
use uuid::Uuid;
...
fn test_create_update_dataset_custom() {
    ...
    /// A subset of dataset fields that we can easily check for equality
    ...
    // Helper to normalize moon form field IDs for comparison since they're server-generated
    fn normalize_moon_form_ids(mut label_def: LabelDef) -> LabelDef {
        if let Some(ref mut moon_forms) = label_def.moon_form {
            for moon_form in moon_forms {
                // Replace server-generated IDs with predictable values for comparison
                moon_form.field_id = "normalized_field_id".to_owned();
                moon_form.id = "normalized_id".to_owned();
                moon_form.field_type_id = "normalized_type_id".to_owned();
            }
        }
        label_def
    }
    ...
    impl From<Dataset> for DatasetInfo {
        fn from(dataset: Dataset) -> DatasetInfo {
            DatasetInfo {
                ...
                label_defs: dataset.label_defs.into_iter().map(normalize_moon_form_ids).collect(),
                label_groups: dataset.label_groups.into_iter().map(|mut group| {
                    group.label_defs = group.label_defs.into_iter().map(normalize_moon_form_ids).collect();
                    group
                }).collect(),
            }
        }
    ...
    // Force recompilation - API returns Some("") for title/instructions, None for trainable
    let mut expected_dataset_info = DatasetInfo {
        ...
        label_defs: vec![
            ...
            LabelDef {
                name: "foo".to_owned(),
                instructions: Some("Long label description".to_owned()),
                external_id: Some("ext id".to_owned()),
                pretrained: Some(Box::new(BuiltinLabelDef {
                    id: "0000000000000001".to_owned(),
                    name: BuiltinLabelDefName::Autogenerated,
                    description: None,
                    properties: None,
                })),
                title: Some("Title Me".to_owned()),
                moon_form: Some(vec![MoonFormFieldDef {
                    field_id: "normalized_field_id".to_owned(),
                    id: "normalized_id".to_owned(),
                    field_type_id: "normalized_type_id".to_owned(),
                    name: "luna".to_owned(),
                    kind: Some("ent".to_owned()),
    ...
    let mut updated_expected = expected_dataset_info.clone();
    "updated title".clone_into(&mut updated_expected.title);
    assert_eq!(get_dataset_info(), updated_expected);
    ...
    expected_dataset_info.source_ids = vec![source.id];
    assert_eq!(get_dataset_info(), expected_dataset_info);
...
fn test_create_dataset_with_source() {
    ...
    assert_eq!(&dataset_info.owner, dataset.owner());
    assert_eq!(&dataset_info.name, dataset.name());
    ...
    let source_output = cli.run([
        ...
        "get",
        "sources",
        dataset_info.source_ids.first().unwrap(),
    ]);
    let source_info: Source = serde_json::from_str(source_output.trim()).unwrap();
    assert_eq!(&source_info.owner, source.owner());
    assert_eq!(&source_info.name, source.name());
}
#[test]
fn test_create_dataset_with_gen_ai() {
    ...
    assert_eq!(
        &dataset_gen_ai_false_info.owner,
        dataset_gen_ai_false.owner()
    );
    assert_eq!(
        &dataset_gen_ai_false_info.name,
    ...
    let dataset_info: Dataset = serde_json::from_str(output.trim()).unwrap();
    assert_eq!(&dataset_info.owner, dataset_gen_ai.owner());
    assert_eq!(&dataset_info.name, dataset_gen_ai.name());
    assert!(dataset_info._dataset_flags.contains(&DatasetFlag::Gpt4));
}
#[test]
fn test_create_dataset_with_zero_shot() {
    ...
    assert_eq!(
        &dataset_zero_shot_false_info.owner,
        dataset_zero_shot_false.owner()
    );
    assert_eq!(
        &dataset_zero_shot_false_info.name,
    ...
    let dataset_info: Dataset = serde_json::from_str(output.trim()).unwrap();
    assert_eq!(&dataset_info.owner, dataset_zero_shot.owner());
    assert_eq!(&dataset_info.name, dataset_zero_shot.name());
    assert!(dataset_info
        ._dataset_flags
        .contains(&DatasetFlag::ZeroShotLabels));
}
...
fn test_create_dataset_with_external_llm() {
    ...
    assert_eq!(&dataset_ellm_false_info.owner, dataset_ellm_false.owner());
    assert_eq!(&dataset_ellm_false_info.name, dataset_ellm_false.name());
    ...
    let dataset_info: Dataset = serde_json::from_str(output.trim()).unwrap();
    assert_eq!(&dataset_info.owner, dataset_ellm.owner());
    assert_eq!(&dataset_info.name, dataset_ellm.name());
    assert!(dataset_info
        ._dataset_flags
        .contains(&DatasetFlag::ExternalMoonLlm));
}
...
fn test_create_dataset_with_no_flags() {
    ...
    assert_eq!(&dataset_qos_none_info.owner, dataset_qos_none.owner());
    assert_eq!(&dataset_qos_none_info.name, dataset_qos_none.name());
...
fn test_create_dataset_with_qos() {
    ...
    assert_eq!(&dataset_qos_false_info.owner, dataset_qos_false.owner());
    assert_eq!(&dataset_qos_false_info.name, dataset_qos_false.name());
    ...
    let dataset_info: Dataset = serde_json::from_str(output.trim()).unwrap();
    assert_eq!(&dataset_info.owner, dataset_qos.owner());
    assert_eq!(&dataset_info.name, dataset_qos.name());
    assert!(dataset_info._dataset_flags.contains(&DatasetFlag::Qos));
}
...
fn test_create_dataset_model_family() {
    ...
    assert_eq!(&dataset_info.owner, dataset.owner());
    assert_eq!(&dataset_info.name, dataset.name());
    assert_eq!(dataset_info.model_family, ModelFamily::German);
}
...
fn test_create_dataset_wrong_model_family() {
    ...
    assert!(stderr.contains("Invalid request - Unsupported model family: non-existent-family"));
}
...
fn test_create_dataset_copy_annotations() {
    ...
    let output = cli
        ...
        .args([
            "create",
            "dataset",
            &format!("--copy-annotations-from={}", dataset1_info.id),
            &format!("{}/test-dataset-{}", TestCli::project(), Uuid::new_v4()),
        ])
        .output()
        .unwrap();
