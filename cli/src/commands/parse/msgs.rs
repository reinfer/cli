...
pub fn parse(config: &Configuration, args: &ParseMsgArgs) -> Result<()> {
    ...
    if msg_paths.is_empty() {
        return Err(anyhow!("No MSG files found in directory: {}", directory.display()));
    }
...
mod tests {
    ...
    fn test_read_msg_to_raw_email_document_unicode() {
        ...
        let actual_document = read_msg_to_raw_email_document(&PathBuf::from("tests/samples/unicode.msg"))
