...
use log::info;
use reinfer_client::{Client, DatasetFullName, EntityDef, LabelDef, NewDataset, SourceIdentifier};
...
pub fn create(client: &Client, args: &CreateDatasetArgs, printer: &Printer) -> Result<()> {
    ...
    let new_label_defs: Vec<_> = label_defs
        .iter()
        .map(|label_def| label_def.to_new())
        .collect();
    ...
    let dataset = client
        .create_dataset(
            ...
            NewDataset {
                ...
                label_defs: if new_label_defs.is_empty() {
                    None
                } else {
                    Some(&new_label_defs[..])
                },
