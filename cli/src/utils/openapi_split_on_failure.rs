...
{
    debug!("Attempting batch {} with {} items", operation_name, request.item_count());
    ...
    match api_call(request.clone()) {
        ...
        Err(error) if should_split_request(&error) => {
            debug!("Batch {} failed with splittable error, attempting to split", operation_name);
        ...
        Err(error) => {
            ...
            Err(convert_openapi_error(error)).with_context(|| {
                format!("Failed to execute {} (non-splittable error)", operation_name)
            })
        }
    }
}
    ...
    F: Fn(RequestT) -> Result<ResponseT, OpenApiError<ErrorT>>,
{
    ...
    debug!("Split {} into {} individual requests", operation_name, total_requests);
    ...
    for (index, individual_request) in individual_requests.into_iter().enumerate() {
        match api_call(individual_request) {
            Ok(response) => {
                ...
                debug!("Individual {} request {}/{} succeeded", operation_name, index + 1, total_requests);
            }
            Err(error) => {
                num_failed += 1;
                debug!("Individual {} request {}/{} failed: {:?}", operation_name, index + 1, total_requests, error);
            }
        }
    }
    
    let num_succeeded = total_requests - num_failed;
    debug!("Split {} completed: {} succeeded, {} failed", operation_name, num_succeeded, num_failed);
