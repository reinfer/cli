...
update_spec() {
    ...
    if ! "$SCRIPT_DIR/update-spec.sh" --ref "$branch" -o "$SPEC_FILE"; then
...
preprocess_spec() {
    ...
    if ! python3 "$SCRIPT_DIR/preprocess-spec.py" "$SPEC_FILE" "$PREPROCESSED_SPEC"; then
...
# Function to run gen-client.sh
generate_client() {
    log_info "Generating Rust client from preprocessed spec"
    
    if ! "$SCRIPT_DIR/gen-client.sh" -i "$PREPROCESSED_SPEC" -o "$RUST_CLIENT_DIR"; then
...
check_prerequisites() {
    ...
    local required_scripts=("update-spec.sh" "preprocess-spec.py" "gen-client.sh")
    ...
    # Check if Docker is available (required for gen-client.sh)
...
main() {
    ...
    echo
    log_info "Generated Rust client: $RUST_CLIENT_DIR"
    echo
}
