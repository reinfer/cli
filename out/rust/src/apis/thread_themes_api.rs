...
/// struct for typed errors of method [`get_thread_themes`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetThreadThemesError {
...
pub async fn get_thread_themes(configuration: &configuration::Configuration, owner: &str, dataset_name: &str, get_thread_themes_request: models::GetThreadThemesRequest) -> Result<models::GetThreadThemesResponse, Error<GetThreadThemesError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/preview/datasets/{owner}/{dataset_name}/thread-themes", local_var_configuration.base_path, owner=crate::apis::urlencode(owner), dataset_name=crate::apis::urlencode(dataset_name));
    ...
    local_var_req_builder = local_var_req_builder.json(&get_thread_themes_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetThreadThemesError> = serde_json::from_str(&local_var_content).ok();
