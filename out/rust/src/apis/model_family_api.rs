...
/// struct for typed errors of method [`get_pretrained_labels`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPretrainedLabelsError {
...
pub async fn get_pretrained_labels(configuration: &configuration::Configuration, model_family: &str) -> Result<models::GetPretrainedLabelsResponse, Error<GetPretrainedLabelsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/_private/model-family/{model_family}/pretrained-labels", local_var_configuration.base_path, model_family=crate::apis::urlencode(model_family));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetPretrainedLabelsError> = serde_json::from_str(&local_var_content).ok();
