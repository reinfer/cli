...
pub struct GetStreamResultsResponse {
    /// `ok` if request was successful, `error` otherwise
    #[serde(rename = "status")]
    pub status: Status,
    /// New comments since last advance, with their predictions if the stream has a model selected.
    #[serde(rename = "results")]
    pub results: Vec<models::StreamResult>,
    #[serde(rename = "model", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub model: Option<Option<Box<models::UserModel>>>,
    /// How many comments were downloaded but filtered out by the stream's filter.
    #[serde(rename = "num_filtered")]
    pub num_filtered: i32,
    /// True if there are more results after this batch---calling `fetch` after `advance` will return more results.
    #[serde(rename = "more_results")]
    pub more_results: bool,
    /// An opaque token that can be used to advance the trigger past the results in `results`. Individual results also contain `continuation`-s, but if you want to advance past ALL the results from this response, use this value as it will potentially advance past additional filtered values.
    #[serde(rename = "continuation")]
    pub continuation: String,
}

impl GetStreamResultsResponse {
    pub fn new(status: Status, results: Vec<models::StreamResult>, num_filtered: i32, more_results: bool, continuation: String) -> GetStreamResultsResponse {
