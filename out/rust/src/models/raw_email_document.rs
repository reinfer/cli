...
pub struct RawEmailDocument {
    /// An object containing the main body of the email
    #[serde(rename = "raw_email")]
    pub raw_email: Box<models::RawEmail>,
    ///  Identifies a comment uniquely. If no comment ID is provided, Re:infer will generate the comment ID from the email's message ID. The format is described in the Comment Reference.
    ...
    pub user_properties: Option<std::collections::HashMap<String, String>>,
}

impl RawEmailDocument {
    pub fn new(raw_email: models::RawEmail) -> RawEmailDocument {
        RawEmailDocument {
            raw_email: Box::new(raw_email),
            comment_id: None,
