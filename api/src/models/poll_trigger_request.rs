...
pub struct PollTriggerRequest {
    /// The number of comments to fetch from this trigger
    #[serde(rename = "size")]
    pub size: i32,
    /// How many comments to skip if if they don't match the trigger's comment filter
    #[serde(rename = "max_filtered", skip_serializing_if = "Option::is_none")]
    pub max_filtered: Option<i32>,
}

impl PollTriggerRequest {
    pub fn new(size: i32) -> PollTriggerRequest {
        PollTriggerRequest {
            size,
            max_filtered: None,
        }
