...
pub struct StreamCommentPrediction {
    /// Currently only one taxonomy can be defined per dataset, but for future compatibility search for the taxonomy named `default`.
    #[serde(rename = "taxonomies")]
    pub taxonomies: Vec<models::StreamTaxonomyPrediction>,
    /// The score predictions for the comment, such as Quality of Service or Tone, when these scores are enabled on the dataset. Scores with a value of 0.0 are omitted.
    #[serde(rename = "scores")]
    pub scores: Vec<models::StreamScorePrediction>,
}

impl StreamCommentPrediction {
    pub fn new(taxonomies: Vec<models::StreamTaxonomyPrediction>, scores: Vec<models::StreamScorePrediction>) -> StreamCommentPrediction {
