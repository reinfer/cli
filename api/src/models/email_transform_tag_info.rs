...
pub struct EmailTransformTagInfo {
    #[serde(rename = "text_format")]
    pub text_format: Box<models::TextFormat>,
    /// If this field exists there is an equivalent rich text transform tag. Update the source with this tag here to trigger a rich text migration
    #[serde(rename = "rich_text_upgrade_tag", skip_serializing_if = "Option::is_none")]
    pub rich_text_upgrade_tag: Option<String>,
}

impl EmailTransformTagInfo {
    pub fn new(text_format: models::TextFormat) -> EmailTransformTagInfo {
        EmailTransformTagInfo {
            text_format: Box::new(text_format),
            rich_text_upgrade_tag: None,
        }
    }
}
