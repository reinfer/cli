...
pub struct ThemeComment {
    /// The Communications Mining identifier for a comment (opposed the an external ID for a verbatim in the client system)
    #[serde(rename = "comment_uid")]
    pub comment_uid: String,
    /// Text spans for importance to the cluster and where in the verbatim these spans are located. N.B. Asthere is only one interpretation per verbatim there cannot be importance spans on both subject & body.
    #[serde(rename = "interpretation", skip_serializing_if = "Option::is_none")]
    pub interpretation: Option<Box<models::ThemeCommentInterpretation>>,
}

impl ThemeComment {
    pub fn new(comment_uid: String) -> ThemeComment {
        ThemeComment {
            comment_uid,
            interpretation: None,
        }
