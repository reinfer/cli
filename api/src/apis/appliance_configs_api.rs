...
/// struct for typed errors of method [`get_appliance_config`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetApplianceConfigError {
    Status4XX(models::ErrorResponse),
    Status5XX(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`update_appliance_config`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateApplianceConfigError {
    Status4XX(models::ErrorResponse),
    Status5XX(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub fn get_appliance_config(configuration: &configuration::Configuration, owner: &str, config_key: &str) -> Result<std::path::PathBuf, Error<GetApplianceConfigError>> {
    ...
    let local_var_uri_str = format!("{}/api/_private/appliance-configs/{owner}/{config_key}", local_var_configuration.base_path, owner=crate::apis::urlencode(owner), config_key=crate::apis::urlencode(config_key));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetApplianceConfigError> = serde_json::from_str(&local_var_content).ok();
...
pub fn update_appliance_config(configuration: &configuration::Configuration, owner: &str, config_key: &str, body: std::path::PathBuf) -> Result<models::UpdateApplianceConfigResponse, Error<UpdateApplianceConfigError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<UpdateApplianceConfigError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
